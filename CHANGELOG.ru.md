...
- `supports(url_or_name)` — имя плеера, который обслуживает ссылку, или
  `None`; не бросающий исключение аналог `get_player_class` для тех, кто
  перебирает много ссылок с агрегатора.

- HLS-варианты, у которых звук вынесен в отдельную дорожку
  `#EXT-X-MEDIA:TYPE=AUDIO` (например, fMP4-плейлисты Aniboom), теперь несут её
  в `Stream.audio_url` / `extra["audio_url"]`, а `ffmpeg_args()` подключает её
  вторым входом с явными `-map`. Раньше скачивание варианта качества давало
  файл без звука.
